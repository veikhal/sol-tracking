...
// IMPORTANT: Replace 'https://your-frontend-vercel-app-domain.vercel.app' with the actual domain of your *frontend* Vercel deployment.
// For local development, you might use 'http://localhost:3000' or '*' for testing (but not in production).
const corsOptions = {
  origin: 'https://your-frontend-vercel-app-domain.vercel.app', // <--- UPDATE THIS!
...
async function fetchWalletBalance(walletAddress, solscanApiKey) {
    try {
        ...
        // Add SOL balance
        walletTotal += parseFloat(data.solBalance) || 0;

        // Add token balances
        if (data.tokens) {
            for (const token of data.tokens) {
                if (token.tokenPriceUsd) {
                    const amount = parseFloat(token.tokenAmount?.amount || '0') /
                                   Math.pow(10, parseFloat(token.tokenAmount?.decimals || '0'));
                    walletTotal += amount * parseFloat(token.tokenPriceUsd);
                }
            }
        }
        return walletTotal;
